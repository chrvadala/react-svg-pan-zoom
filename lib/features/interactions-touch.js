...
function onMultiTouch(event, ViewerDOM, tool, value, props) {
  ...
  if (distanceFactor === 1) {
    return (0, _common.set)(value, { mode: _constants.MODE_ZOOMING, pinchPointDistance: pinchPointDistance });
  }

  event.preventDefault();
  return (0, _zoom.zoom)(value, svgPoint.x, svgPoint.y, distanceFactor, {
    mode: _constants.MODE_ZOOMING,
    prePinchMode: value.mode === _constants.MODE_ZOOMING ? value.prePinchMode : value.mode,
...
function getNextValue(event, ViewerDOM, tool, value, props, nextValueFn) {
  ...
  var nextValue = hasTouchPoints ? value : (0, _common.set)(value, { mode: value.prePinchMode });
